"""
algorithms/cql.py — Conservative Q-Learning (CQL)
===================================================
CQL penalises high Q-values for all actions while encouraging the
dataset action, preventing overestimation of OOD action values:

    L_CQL = E[logsumexp(Q(s, ·)) - Q(s, a_dataset)]

Reference: Kumar et al. (2020) "Conservative Q-Learning for Offline RL."
"""
...
def train(cfg: DictConfig) -> None:
    ...
    logger.info(
        "CQL | device=%s | seed=%d | alpha=%.2f | in_dist=%d | out_dist=%d",
        device, cfg.seed, alg.cql_alpha, len(in_dist), len(out_dist),
    )

    use_wandb = cfg.wandb.enabled
    if use_wandb:
        import wandb
        run_name = cfg.wandb.run_name or f"cql_{cfg.substrate}_{cfg.train_mode}_s{cfg.seed}"
        wandb.init(
    ...
    # Discrete CQL is pure Q-learning: there is no policy network to train.
    # The policy is implicit (argmax over the conservative Q-values at eval
    # time). The MoltenpotAgent has an actor head only because IQL/BC need
    # it; for CQL it stays at initialisation and is never read.
    ...
    for global_step in range(cfg.num_updates):
        ...
        # --- Q target (max over next states) ---
        with torch.no_grad():
            q1_tp1, q2_tp1, _, _, _ = target.get_q_v(
                obs_tp1, target.initial_hidden(B).to(device), agent_ids=agent_ids
            )
            v_tp1    = torch.min(q1_tp1.max(dim=-1)[0], q2_tp1.max(dim=-1)[0])
            q_target = rewards + alg.gamma * (1 - dones) * v_tp1

        # --- Q update with CQL regulariser ---
        q1_s, q2_s, _, _, _ = model.get_q_v(
        ...
        # Conservative penalty: logsumexp(Q) - Q(s, a_data)
        cql1 = (torch.logsumexp(q1_s, dim=-1) - q1_val).mean()
        cql2 = (torch.logsumexp(q2_s, dim=-1) - q2_val).mean()
        cql_loss = alg.cql_alpha * (cql1 + cql2)
        ...
        if global_step % cfg.log_interval == 0:
            ...
            logger.info(
                "Step %d | bellman=%.4f | cql=%.4f",
                global_step, bellman_loss.item(), cql_loss.item(),
            )
            log_dict.update({
    ...
    final_path = ckpt_dir / f"cql_s{cfg.seed}_final.pth"
    ...
    if use_wandb:
        import wandb
        art = wandb.Artifact(
            name=f"cql-{cfg.substrate}-{cfg.train_mode}-s{cfg.seed}",
            type="model",
            description=f"CQL final checkpoint | substrate={cfg.substrate} split={cfg.train_mode} seed={cfg.seed}",
        )
