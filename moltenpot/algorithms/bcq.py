...
def _make_bcq_act_fn(model: MoltenpotAgent, tau: float):  # kept for local use only
    """
    Return a BCQ greedy act_fn for evaluate_multi_scenario.

    Masks out actions whose imitator probability is below τ × max_prob,
    then picks the action with the highest Q-value among the remaining set.
    """
    device = next(model.parameters()).device

    def act_fn(
        obs_t: torch.Tensor, h_state: torch.Tensor
    ) -> Tuple[np.ndarray, torch.Tensor]:
        # obs_t shape: (A, 3, 88, 88) — unsqueeze T dimension for get_q_v
        q1, q2, _, logits, h_state = model.get_q_v(
            obs_t.unsqueeze(1), h_state
        )
        q_min    = torch.min(q1, q2).squeeze(1)           # (A, num_actions)
        probs    = F.softmax(logits.squeeze(1), dim=-1)    # (A, num_actions)
        max_prob = probs.max(dim=-1, keepdim=True)[0]
        mask     = (probs / max_prob) > tau
        q_min[~mask] = -1e10
        actions_np = q_min.argmax(dim=-1).cpu().numpy()
        return actions_np, h_state
